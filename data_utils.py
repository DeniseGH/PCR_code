...
def process_data(data, scmm_true):
    if data == "compas":
        ...
        return process_linear_data(scmm_true)
    else:
...
def process_linear_data(scmm_true):
    X, y = scmm_true.generate(1000)
    actionable_indices = [0,1,2]
    increasing_ids = []
    decreasing_ids = []
    feature_limits = torch.tensor(np.array([[-1, 1]]).repeat(3, axis=0) * 1e10)
    constraints = {'actionable': actionable_indices, 'increasing': increasing_ids,
                   'decreasing': decreasing_ids, 'limits': feature_limits}
    return X, y, constraints
