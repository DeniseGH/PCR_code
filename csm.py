...
class StructuralCausalModel:
        ...
        for node, model in assignment.items():
            if model is None:
                ...
                parents = [
                    ...
                    if parent != "n_samples" and parent != "thetas"
                ]
    ...
    def sample(self, n_samples=100, set_values=None, type_of_intervention="soft"):
        """
        Sample from CSM
        ...
        # If set_values is None, initialize it as an empty dictionary
        # if set_values is None:
        #    set_values = dict()

        # Iterate over nodes in topological order
        for node in nx.topological_sort(self.cgm.dag):
            # Get the causal model for the current node
            c_model = self.assignment[node]

            # If the causal model is None, it means the variable is intervened
            if c_model is None:
                # print("you intervened on the variable: ", node)
                # Check if the number of provided values matches n_samples
                assert len(set_values[node]) == n_samples
                # Store intervened values in samples
                samples[node] = np.array(set_values[node])
            else:
                # If the variable is not intervened, generate samples based on its parents' samples
                parent_samples = {
                    parent: samples[parent]  # Get samples of parent variables
                    for parent in c_model.parents  # Iterate over parent variables
                }
                parent_samples["n_samples"] = n_samples  # Add n_samples to parent_samples
                # print("generating samples of: ", node, "using: ", c_model)
                samples[node] = c_model(**parent_samples)  # Generate samples for the current variable

                if set_values is not None:
                    if node in set_values:
                        if type_of_intervention == "hard":
                            samples[node] = np.array(set_values[node])
                        else:
                            samples[node] += np.array(set_values[node])

        # Convert the dictionary of samples into a pandas DataFrame and return it
